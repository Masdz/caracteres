...
var authN = require(__dirname + '/toke/aut');
...
//Cargar el HTML para subir archivos
//Aut recibe TOKEN, auth para verificar el TOKEN enviado 
app.get('/cargaA', [authN.autenticacion], function (req, res, next) {
	...
	firebase.auth().getUserByEmail(email).then(function (user) {
		...
		if (upass == rpass) {
			firebase.auth().createCustomToken(llave).then(function (token) {
				res.status(200).send('{"Token":' + token + ',"Acceso"' + true + '}');
